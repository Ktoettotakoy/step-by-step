...
export default function GameScreen() {
    ...
    useEffect(() => {
        const fetchAndGenerateAllCards = async () => {
            try {
                ...
                // 1. Fetch words from your new API route that uses analyzeSampleImages
                console.log("Fetching words from sample images via API...");
                const response = await fetch('/api/analyze-samples'); // Your new API endpoint
                if (!response.ok) {
                    const errorData = await response.json();
                    throw new Error(errorData.error || `HTTP error! status: ${response.status}`);
                }
                const words = await response.json();
                console.log("Words fetched from samples:", words);


                if (!words || words.length === 0) {
                    throw new Error("No words generated from sample images.");
                }
                ...
                console.log("All generated game cards (words + new images):", allGeneratedCards);

            } catch (err) {
                console.error("Error generating all game data:", err);
                setFeedbackMessage("Failed to load game data: " + err.message + " Please refresh.");
    ...
    //             // playVoiceFromText function is used here. Make sure it's imported correctly.
    //             playVoiceFromText(currentCard.word);
    //         }, 500);

    //         return () => clearTimeout(playTimeout);
    //     }
    // }, [gamePhase, currentCard]); // Removed isGeneratingImage from dependencies as it's not relevant here anymore
